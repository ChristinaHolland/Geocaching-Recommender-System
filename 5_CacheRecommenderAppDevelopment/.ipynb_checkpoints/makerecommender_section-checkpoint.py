...
print('What number cache should I start on?')
section_no = input()

print('Thank you. Loading data ...')

full = pd.read_csv('../data/working_files/strippeddata.csv')
df = pd.read_csv('../data/working_files/code_name_label.csv')
xy = pd.read_csv('../data/working_files/coords.csv')
rec1 = pd.read_csv('../data/working_files/recommender1.csv') # based on non-text aspects of the cache, + 
...
print('Data loaded, ready to begin ...')
...
a = 0 + 1000*section_no
b = 1000 + 1000*section_no
if b>df.shape[0]: b=df.shape[0]
section_ndx = list(range(a,b))

output_strings = []
for n in section_ndx:
    code = codes[n]
    ...
    L = len(section_ndx)
    pc = m*100/L
    print(f'Cache #{m}: {code}, {pc}% of the {a}-{b} section completed ...')   
    
section_df = pd.DataFrame({'ndx': section_ndx, 'output_string': output_string})

filename1 = f'../data/final_files/rec_file_{a}_{b-1}.csv'
section_df.to_csv(filename1,index=False)
